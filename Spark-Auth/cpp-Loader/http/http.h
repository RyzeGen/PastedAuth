...
namespace http
{
	...
	request* get(std::string url, req_headers h_data = DEFAULT_HEADERS, USHORT port = 80);
	request* post(std::string url, const post_data& pdata, req_headers h_data = DEFAULT_POST_HEADERS, USHORT port = 80);
}
#endif
